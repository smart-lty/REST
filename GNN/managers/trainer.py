...
class Trainer():
    def __init__(self, params, graph_classifier, train_data, valid_evaluator=None):
        ...
        if params.optimizer == "SGD":
            self.optimizer = optim.SGD(filter(lambda p: p.requires_grad, model_params), lr=params.lr, momentum=params.momentum, weight_decay=self.params.l2)
    ...
    def train_epoch(self):
        ...
        if self.valid_evaluator:
            ...
            logging.info('\nValidation Performance:' + str(result) + ' in ' + str(time.time() - tic) + ' s ')
        ...
        train_result = {'auc': auc, 'auc_pr': auc_pr}
        logging.info('\Training Performance:' + str(train_result) + ' in ' + str(time.time() - tic) + ' s ')
    ...
    def train(self):
        ...
        for epoch in range(1, self.params.num_epochs + 1):
            ...
            logging.info(f'Epoch {epoch} with loss: {loss}, best validation MRR: {self.best_metric}, weight_norm: {weight_norm} in {time_elapsed}')
